            ...
            for (const file of readdirSync('./src/components/' + directory).filter((f) => f.endsWith('.js'))) {
                try {
                    ...
                    if (module.__type__ === 3) {
                        ...
                        // !info - for the autocomplete stuff
                        if (!module.commandName || !module.run) {
                            error('Unable to load the autocomplete component ' + file);
                            continue;
                        }

                        const commandNames = Array.isArray(module.commandName)
                            ? module.commandName
                            : [module.commandName];

                        for (const commandName of commandNames) {
                            info("[Registerd]",commandName)
                            this.client.collection.components.autocomplete.set(commandName, module);
                        }
